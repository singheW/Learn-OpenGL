...
//"in vec3 ourColor;\n"//从顶点着色器传入颜色
//"in vec2 TexCoord;\n"
//"uniform float mixValue;\n"//练习四：使用一个uniform变量作为mix函数的第三个参数来改变两个纹理可见度，使用上和下键来改变箱子或笑脸的可见度：
//"uniform sampler2D texture1;\n"//纹理单元GL_TEXTURE0默认总是被激活
//"uniform sampler2D texture2;\n"
//"void main()\n"
//"{\n"
////"FragColor = texture(ourTexture,TexCoord)*vec4(ourColor,1.0);\n"// texture(纹理采样器, 对应的纹理坐标);
//"FragColor = mix(texture(texture1,TexCoord),texture(texture2,1-TexCoord),mixValue);\n"//练习第一题：1-TexCoord)
//"}\n\0";
//
//int main()
...
//        //     ---- 位置 ----       ---- 颜色 ----     - 纹理坐标 -
//             0.5f,  0.5f, 0.0f,   1.0f, 0.0f, 0.0f,   2.0f, 2.0f,   // 右上    //练习二：设定一个从0.0f到2.0f范围内的（而不是原来的0.0f到1.0f）纹理坐标。
...
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_MIRRORED_REPEAT);//练习二：尝试用不同的纹理环绕方式
...
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT);
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);//练习三：试使用GL_NEAREST的纹理过滤方式让像素显示得更清晰
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
//
...
//        glBindTexture(GL_TEXTURE_2D, texture2);
//
//        float mixVal = 0.5f;//练习四：使用一个uniform变量作为mix函数的第三个参数来改变两个纹理可见度，使用上和下键来改变箱子或笑脸的可见度：
...
//            mixVal = 1.0f;
//        }
//        int mixValue = glGetUniformLocation(shaderProgram, "mixValue");
