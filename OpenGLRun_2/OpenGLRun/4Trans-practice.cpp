...
//颜色属性
    ...
    while (!glfwWindowShouldClose(WindowPtr))
    {
        ...
        transform = glm::translate(transform, glm::vec3(0.5f, -0.5f, 0.0f));//练习1：使用应用在箱子上的最后一个变换，尝试将其改变为先旋转，后位移。旋转点变为中心
        // 本质就是矩阵乘法不满足交换律，两个矩阵与位置相乘，矩阵的顺序交换会导致结果不同。
        //旋转矩阵是围绕坐标原点旋转的。所以旋转之前确保你的旋转中心和坐标原点重合。 开始的时候，坐标中心就是图片中心，此时旋转最省事
     ...
     //Transform第三步:把变换矩阵传递给着色器：
        ...
        transform = glm::mat4(1.0f);//练习二：尝试再次调用glDrawElements画出第二个箱子，只使用变换将其摆放在不同的位置。让这个箱子被摆放在窗口的左上角，并且会不断的缩放（而不是旋转）。
        transform = glm::translate(transform, glm::vec3(0.5f, 0.5f, 0.0f));//放置位置
        float scaleAmount = sin(glfwGetTime());//变换函数值
        transform = glm::scale(transform, glm::vec3(scaleAmount, scaleAmount, scaleAmount));//缩放值
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, &transform[0][0]);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);

        transform = glm::mat4(1.0f);//玩耍
        transform = glm::translate(transform, glm::vec3(-0.5f,- 0.5f, 0.0f));//放置位置
        transform = glm::scale(transform, glm::vec3(scaleAmount, 1, 1));//缩放值
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, &transform[0][0]);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        transform = glm::mat4(1.0f);//玩耍
        transform = glm::translate(transform, glm::vec3(-0.5f, 0.5f, 0.0f));//放置位置
        transform = glm::scale(transform, glm::vec3(1, scaleAmount, 1));//缩放值
        glUniformMatrix4fv(transformLoc, 1, GL_FALSE, &transform[0][0]);
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
        // 交换颜色缓冲
